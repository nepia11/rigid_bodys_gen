...
class RBG_PT_MenuAddBonesTools(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.operator(
            RBG_OT_AddBonesOnEdges.bl_idname,
            text="Adding Bones On Edges",
            icon="BONE_DATA",
        )
...
class RBG_OT_AddBonesOnEdges(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for e in elist:
            ...
            bonelist = np.append(
                bonelist, [[bone.name, e.verts[0].index, e.verts[1].index]], axis=0
            )
...
classes = [RBG_PT_MenuAddBonesTools, RBG_OT_AddBonesOnEdges]
